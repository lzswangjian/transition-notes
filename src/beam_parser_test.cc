...
void TestReaderOP(int argc, char *argv[]) {
  // Init Parser Config.
...
void TestEval(int argc, char *argv[]) {
  // Init Parser Config.
  TaskContext *context = new TaskContext();
  TaskSpec *spec = context->mutable_spec();
  ...
  BeamParser *beam_parser = new BeamParser(context);
  BeamParserOutput *beam_parser_output = new BeamParserOutput(context);
  BeamEvalOutput *beam_eval_output = new BeamEvalOutput(context);
  ...
  while (1) {
    int epoch = structured_parser->PredictOneBatch(max_steps, parsed_sentence);
    if (epoch >= num_epoch) {
      break;
    }
  }
  for (size_t i = 0; i < parsed_sentence.size(); ++i) {
