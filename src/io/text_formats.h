#ifndef SYNTAXNET_TEXT_READER_H_
#define SYNTAXNET_TEXT_READER_H_
...
class CoNLLSyntaxFormat : public DocumentFormat {
  public:
    ...
    bool ReadRecord(ifstream *stream, string *record) override {
      string line;
      record->clear();
    ...
    void ConvertToString(const Sentence &sentence, string *key, string *value) override {
      *key = sentence.docid();
      vector<string> lines;
